...
def create_education_fields(num_education):
    ...
    for i in range(num_education):
        ...
        tk.Label(app, text=f"Education {i + 1} Degree:").grid(row=row_offset, column=0, sticky="e", padx=(5, 0))
        degree_entry = tk.Entry(app)
        degree_entry.grid(row=row_offset, column=1)
        edu_degree_entries.append(degree_entry)

        tk.Label(app, text=f"Education {i + 1} Major:").grid(row=row_offset + 1, column=0, sticky="e", padx=(5, 0))
        major_entry = tk.Entry(app)
        major_entry.grid(row=row_offset + 1, column=1)
        edu_major_entries.append(major_entry)

        tk.Label(app, text=f"Education {i + 1} Institution:").grid(row=row_offset + 2, column=0, sticky="e", padx=(5, 0))
        ...
        tk.Label(app, text=f"Education {i + 1} Years:").grid(row=row_offset + 3, column=0, sticky="e", padx=(5, 0))
...
def create_work_experience_fields(num_experiences):
    ...
    for i in range(num_experiences):
        ...
        tk.Label(app, text=f"Work Experience {i + 1} Role:").grid(row=row_offset, column=0, sticky="e", padx=(5, 0))
        role_entry = tk.Entry(app)
        role_entry.grid(row=row_offset, column=1)
        work_role_entries.append(role_entry)

        tk.Label(app, text=f"Work Experience {i + 1} Company:").grid(row=row_offset + 1, column=0, sticky="e", padx=(5, 0))
        company_entry = tk.Entry(app)
        company_entry.grid(row=row_offset + 1, column=1)
        work_company_entries.append(company_entry)

        tk.Label(app, text=f"Work Experience {i + 1} Years:").grid(row=row_offset + 2, column=0, sticky="e", padx=(5, 0))
        years_entry = tk.Entry(app)
